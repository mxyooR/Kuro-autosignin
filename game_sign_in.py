...
class GameSignIn:
    ...
    def get_sign_reward(
        self, game_type: GameType, role_id: str, user_id: str
    ) -> Optional[str]:
        """
        获取签到奖励信息
        ...
        try:
            ...
            response = self.client.game_post(
                API.GAME_SIGN_RECORD, data, raise_on_error=False
            )
    ...
    def check_replenish_count(
        ...
        try:
            ...
            response = self.client.game_post(
                API.GAME_SIGN_INIT, data, raise_on_error=False
            )
    ...
    def replenish_sign(
        self, game_type: GameType, role_id: str, user_id: str, month: str
    ) -> SignInResult:
        """
        执行补签
        :param game_type: 游戏类型
        ...
        try:
            ...
            response = self.client.game_post(
                API.GAME_REPLENISH_SIGN, data, raise_on_error=False
            )
    ...
    def sign_in(
        ...
        try:
            ...
            response = self.client.game_post(
                API.GAME_SIGN_IN, data, raise_on_error=False
            )
            ...
            if auto_replenish and result.status == ResponseStatus.SUCCESS:
                ...
                if omission_count > 0:
                    replenish_result = self.replenish_sign(
                        game_type, role_id, user_id, month
                    )
