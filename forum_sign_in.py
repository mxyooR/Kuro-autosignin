...
class ForumSignIn:
    ...
    def get_post_detail(self, post_id: str) -> bool:
        ...
        """
        try:
            data = {"isOnlyPublisher": "0", "postId": post_id, "showOrderTyper": "2"}

            response = self.client.bbs_post(
                API.FORUM_POST_DETAIL, data, raise_on_error=False
            )
    ...
    def forum_sign_in(self) -> bool:
        ...
        try:
            data = {"gameId": "2"}
            response = self.client.bbs_post(
                API.USER_SIGN_IN, data, raise_on_error=False
            )
    ...
    def get_task_list(self) -> Optional[List[dict]]:
        ...
        try:
            data = {"gameId": "0"}
            response = self.client.bbs_post(
                API.TASK_PROCESS, data, raise_on_error=False
            )
    ...
    def execute_tasks(self) -> SignInResult:
        ...
        try:
            ...
            if not tasks:
                return SignInResult(
                    status=ResponseStatus.FAILED, message="获取任务列表失败"
                )
            ...
            return SignInResult(status=ResponseStatus.SUCCESS, message=result_message)
